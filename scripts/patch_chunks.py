...
scripts/extract_chapter.py regenerates chunks.json from scratch, so run this after
every extraction:

    python -m scripts.extract_chapter <pdf> --niveau 2eme --chapitre 1
    python -m scripts.patch_chunks
    python -m app.rag.rag_store --chunks chunks.json --reset
