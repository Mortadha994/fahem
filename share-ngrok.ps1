...
# See docker-compose.ngrok.yml for the .env entries it needs.
#
...
if ($missing) {
    ...
    Write-Host "See the header of docker-compose.ngrok.yml."
    exit 1
}
...
if ($healthy) {
    ...
    Write-Host "Not answering yet at $url - check: docker compose -f docker-compose.yml -f docker-compose.ngrok.yml logs ngrok" -ForegroundColor Yellow
}
Write-Host "Google sign-in needs $url in Google Cloud Console > Credentials > your OAuth client > Authorised JavaScript origins."
Write-Host "Stop sharing:  docker compose -f docker-compose.yml -f docker-compose.ngrok.yml stop ngrok"
