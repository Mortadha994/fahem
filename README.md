...
| **AI monitoring** | `app/llm/llm_usage.py` records each Groq call (tokens, latency, queue wait, 429s, outcome) to `llm_calls` from a background writer; `app/routes/admin/admin_monitoring.py` serves `/admin/monitoring` — per-model temperature (tokens/min, tokens/day, queue), 24 h KPIs, per-kind p50/p95, recent failures. Each row also carries the `user_id` that caused it, so spend can be read per account. Token counts are Groq's own `usage` figures, never an estimate made here. |
| **What the budget gauge is measured against** | Groq publishes its daily ceiling only inside a 429 body, so the denominator is either a figure Groq stated or `GROQ_TPD_LIMIT`, the compiled-in default — and the console says which (*plafond estimé*). The stated ceiling is kept for 30 days rather than a day: it belongs to the account's tier, not to the date, and daily-limit 429s can be a week apart. The numerator is a rolling 24 h sum of `llm_calls`, raised to Groq's own figure when a 429 reported a higher one within the hour, because Groq also counts calls this app never recorded. |
