...
Reads chunks.json (output of scripts/extract_chapter.py), embeds each chunk with a
...
2. Chroma returned a cosine *distance* (1 - similarity); Qdrant returns the
   cosine *similarity* directly. That conversion lives in app/rag/retrieval.py where
...
def load_chunks(path: Path = DEFAULT_CHUNKS) -> list[dict[str, Any]]:
    ...
    coupled to one exact shape of scripts/extract_chapter.py's output.
    """
    if not path.exists():
        raise SystemExit(
            f"{path} not found. Run scripts/extract_chapter.py first, e.g.:\n"
            "  python -m scripts.extract_chapter path/to/chapter.pdf "
...
def make_id(text: str, meta: dict[str, Any]) -> str:
    ...
    Hit.chunk_id, in app/rag/context.py's pinned-vs-retrieved dedup, and in the API's
