...
Google flow in app/auth/auth.py. Four things to know before the code:
...
1. Identity is the only thing that differs. Once a password is verified, the
   ...
   create_session_token() and set_session_cookie(), imported from app/auth/auth.py
...
from app.auth import emails
from app.auth.auth import (  # the shared mechanism
    UserOut,
    bind_user,
...
from app.core.models import AuthToken, User
