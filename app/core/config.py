...
Backwards compatibility note: app/llm/generate.py and app/rag/rag_store.py re-export the
names they previously defined, so existing imports elsewhere
(`from generate import GROQ_MODEL, GROQ_URL` in app/llm/llm_stream.py,
`from rag_store import QDRANT_URL` in app/rag/context.py/test_retrieval.py)
...
# Reads photos and scanned PDFs of an exercise (app/routes/attachments.py). GROQ_MODEL is
...
# --- Groq queue (app/llm/llm_queue.py) ------------------------------------------------
#
...
# --- AI monitoring (app/llm/llm_usage.py, /admin/monitoring) ----------------------------
#
...
# that were previously hardcoded in app/main.py. 5174 as well as 5173 because
...
# chat messages. Nothing reads this yet beyond app/core/db.py and Alembic - server-side
# persistence exists but is not wired into any endpoint (see app/core/db.py's docstring).
#
# The default targets localhost, which is what a developer running scripts on
...
# Empty means "auth is not configured" - app/auth/auth.py refuses to verify a token at
...
#
# Note: app/rag/context.py, app/llm/generate.py and tests/test_retrieval.py each still carry their
...
# app/routes/chapters.py or an endpoint to point at a different file.
#
...
# real generation legitimately can (app/llm/generate.py uses 300s).
...
# --- chat attachments (app/routes/attachments.py) ------------------------------------------
