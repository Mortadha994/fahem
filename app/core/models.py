...
session persistence) have a foundation to build on. `app/main.py`'s endpoints,
...
Flat module, not a `db/` package: the rest of the repo is flat (app/core/config.py,
app/grading/checker.py, app/llm/gatekeeper.py), and the whole data layer is this file plus app/core/db.py.
...
# free; the column exists so the Groq queue (app/llm/llm_queue.py) can already read a
...
class ChatSession(Base):
    ...
    # Bumped by every save (app/routes/chat_history.py). A save sent from a stale copy -
...
# --- AI monitoring ----------------------------------------------------------------
#
# One row per Groq call, written by app/llm/llm_usage.py, read by the admin console's
...
# path (app/routes/chapters.py's constant, the bind-mounted PDF, sample_problems.json,
# context.PINS and scripts/patch_chunks.py), because re-extracting it would throw away
...
class UploadedChapter(Base):
    ...
    # Phase 9b: "pdf" (extracted by scripts/extract_chapter.py, then reviewed) or
    # "markdown" (a course written to docs/modele-cours.md, read exactly by
    # app/rag/course_markdown.py). For a markdown chapter the student-facing PDF is a
...
class ChapterChunk(Base):
    ...
    # Same vocabulary scripts/extract_chapter.py emits: prose | table | exercice.
...
# the daily budget guard, the solve rate limit, ...). app/core/runtime_settings.py owns
# the keys, their defaults and their validation; a row exists only for a key
# that has been changed, so a missing row means "the default from app/core/config.py".
