...
Deliberately minimal and not imported by app/main.py: Phase 0a stands the database
up without wiring it into any request path. The only callers today are Alembic
(via alembic/env.py, for the URL and metadata) and tests/test_db.py.

Kept separate from app/core/models.py so that importing the models - which Alembic must
