"""Grant or revoke the admin role. The ONLY way to do either (Phase 7).
    ...
    docker compose exec backend python promote_admin.py student@example.com
    docker compose exec backend python promote_admin.py student@example.com --demote
    docker compose exec backend python promote_admin.py --list
...
1. An email can match more than one account. users.email is unique only among
   password accounts (models.py): a Google account and a password account are
