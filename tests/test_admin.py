...
scripts/promote_admin.py - including its refusal to leave zero admins.

Same main() + __main__ shape as tests/test_auth.py; importing this module touches
...
def main() -> None:
    ...
    try:
        ...
        with session_scope() as s:
            ...
            from app.core.models import AdminAuditEntry
...
def http_section(run_id: str, created: list[uuid.UUID], *, student: uuid.UUID, admin: uuid.UUID) -> None:
    ...
    from app.auth import emails, password_auth
    from app.core import ratelimit
    from app.core.config import REDIS_URL
    from app.routes.admin import admin as admin_module

    # Production wiring for these routers (app/main.py's lines), without importing
    # app/main.py - which would load the embedding model.
    app = FastAPI()
